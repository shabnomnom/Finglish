...
@app.route('/pronunciation/<farsi_word>')
def pronunciation(farsi_word):
    json_payload = {'url': utils.word_url(farsi_word)}
...
def login_process():
    ...
    #username 
    current_user = User.query().filter(User.email == email_address).one()
...
def lesson_generator(user_id):
    ...
    for word in lesson_one: 
        user_vocab = Vocabulary(user_id=user_id, word_id=word.word_id, lesson_num=1)
...
def show_user_homepage(user_id):
    ...
    return render_template("users/user_homepage.html", first_name=first_name,
                           last_name = last_name, user= user, country=country)


@app.route("/users/<user_id>/<lesson_id>")
def show_lesson_vocabs(user_id, lesson_id):
    """show lesson vocabs """
    ...
    return render_template("lessons.html", vocabs=vocabs)
